        ...
        public async Task<WeaponStatPercentileCache?> Generate(string itemID, string columnName) {
            ...
            using NpgsqlCommand cmd = await _DbHelper.Command(conn, @$"
                SELECT
	                ...
	                percentile_cont(0.00) within group (order by s.{columnName}) AS q0,
	                percentile_cont(0.05) within group (order by s.{columnName}) AS q5,
	                percentile_cont(0.10) within group (order by s.{columnName}) AS q10,
	                percentile_cont(0.15) within group (order by s.{columnName}) AS q15,
	                percentile_cont(0.20) within group (order by s.{columnName}) AS q20,
	                percentile_cont(0.25) within group (order by s.{columnName}) AS q25,
	                percentile_cont(0.30) within group (order by s.{columnName}) AS q30,
	                percentile_cont(0.35) within group (order by s.{columnName}) AS q35,
	                percentile_cont(0.40) within group (order by s.{columnName}) AS q40,
	                percentile_cont(0.45) within group (order by s.{columnName}) AS q45,
	                percentile_cont(0.50) within group (order by s.{columnName}) AS q50,
	                percentile_cont(0.55) within group (order by s.{columnName}) AS q55,
	                percentile_cont(0.60) within group (order by s.{columnName}) AS q60,
	                percentile_cont(0.65) within group (order by s.{columnName}) AS q65,
	                percentile_cont(0.70) within group (order by s.{columnName}) AS q70,
	                percentile_cont(0.75) within group (order by s.{columnName}) AS q75,
	                percentile_cont(0.80) within group (order by s.{columnName}) AS q80,
	                percentile_cont(0.85) within group (order by s.{columnName}) AS q85,
	                percentile_cont(0.90) within group (order by s.{columnName}) AS q90,
	                percentile_cont(0.95) within group (order by s.{columnName}) AS q95,
	                percentile_cont(1.00) within group (order by s.{columnName}) AS q100
                FROM 
                    weapon_stats s
	            WHERE 
                    item_id = @ItemID
